...
class ComplexGoodStrategy(Strategy):
    ...
    def run(self, data: RecordsBucket, timestep: datetime):
        ...
        if timestep == datetime(2023, 11, 2):
            self.broker.buy_long("NVDA", 100, 100)    # Will buy on the 3rd of November at $440.1612972004770
        if timestep == datetime(2023, 12, 13):
            self.broker.sell_long("NVDA", 100)    # Will sell on the 14th of December at $496.75629603239900
        if timestep == datetime(2024, 1, 4):
            self.broker.buy_long("NVDA", 75, 25)    # Will buy on the 5th of January at $484.6199951171880
        if timestep == datetime(2024, 2, 6):
            self.broker.sell_long("NVDA", 200)    # Sell all on the 7th of February at $683.1900024414060

        # AAPL
        if timestep == datetime(2023, 12, 12):
            self.broker.sell_short("AAPL", 200)    # Will sell on the 14th of December at $197.7676470402110
        if timestep == datetime(2024, 1, 4):
            self.broker.buy_short("AAPL", 100)    # Will buy on the 5th of January at $181.75807721578200
        if timestep == datetime(2024, 1, 23):
            self.broker.sell_short("AAPL", 100)    # Will sell on the 24th of January at $194.77147463800900
        if timestep == datetime(2024, 2, 1):
            self.broker.buy_short("AAPL", 200)    # Will buy on the 2nd of February at $179.6307873390830
