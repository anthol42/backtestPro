<%
  ...
  def get_url(dobj: pdoc.Doc):
    return dobj.url(relative_to=module, link_prefix=link_prefix,
                   top_ancestor=not show_inherited_members)
  
  def get_name(dobj: pdoc.Doc):
    return dobj.qualname + ('()' if isinstance(dobj, pdoc.Function) else '')
  
  def hierarchical_link(dobj: pdoc.Doc):
    modules = [dobj.module]
    m = dobj.module
    while m.supermodule is not None:
      modules.append(m.supermodule)
      m = m.supermodule
    out = "<div>"
    out += ".".join([f'<a title="{mod.refname}" href="{get_url(mod)}">{get_name(mod).split(".")[-1]}</a>' for mod in reversed(modules)])
    return out + "</div>"
...
<%def name="show_module(module)">
  ...
  ##   </nav>
  ## % endif
  % if len(module.name.split(".")) > 2:
      <h1 class="title">Module <code>${module.name.split(".")[1]}</code></h1>
  % else: 
...
<%def name="module_index(module)">
  ...
  <nav id="sidebar">
    ...
    <li><h3>Path</h3>
      <ul>
        <li><code>${hierarchical_link(module)}</code></li>
      </ul>
    </li>
    % endif
    ...
    <li><h3><a href="#header-classes">Classes</a></h3>
      ...
      % for c in classes:
        ...
        <h4 class="accordion-header">
          <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_${c|u}" aria-expanded="false" aria-controls="collapse_${c|u}" onclick="navigateToUrl('${get_url(c)}')">
            <code>${get_name(c)}</code>
          </button>
        </h4>
        <%
        ...
        %>
        <div id="collapse_${c|u}" class="accordion-collapse collapse" data-bs-parent="#classes-accordion">
